	...
	void OnGUI(){
		...
		for (int i = 0; i < rowY; i++) {
			...
			for (int j = 0; j < rowX; j++) {
				...
				int levInt = ((int)((i * rowX) + j));
				string levString = levInt.ToString();
				if( GameControl.controller.getLastCompletedPack() + 1 < levInt){
					levString += "L";
				}
				GUIStyle customButton = new GUIStyle("button");
				customButton.fontSize = 30;
				if(GUI.Button(new Rect(curPos.x, curPos.y ,buttonWidth, buttonHeight), levString, customButton)){
					//if (GameControl.controller.getLastCompletedPack () + 1 >= levInt) { uncomment
						GameControl.controller.setCurrentPack (levInt);
						GameControl.controller.changeToScene ("LevelSelection");
					//}
				}
